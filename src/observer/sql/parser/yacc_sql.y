...
%union {
  ...
  float floats;
	char *position;
  struct ExpressionNode *express_node;
}
...
select_attr:
    ...
    | expr attr_list {
			...
			selects_append_attr_expr(&CONTEXT->ssql->sstr.selection, $1);
		}
  	// | ID DOT ID attr_list {
	// 		RelAttr attr;
...
attr_list:
    ...
    | COMMA expr attr_list {
			...
			// selects_append_attribute(&CONTEXT->ssql->sstr.selection, &attr);
			selects_append_attr_expr(&CONTEXT->ssql->sstr.selection, $2);
...
expr:
      expr PLUS expr{
			...
			ExpressionNode *expression = expression_init($1, $3, NULL, NULL);
	...
	| PLUS expr	{
			fprintf(stdout, "+expr\n");
			ExpressionNode *expression = expression_init($2, NULL, NULL, NULL);
	...
	| ID {
			...
			RelAttr attr;
			relation_attr_init(&attr, NULL, $1);
			$$ = expression_init( NULL, NULL, &attr, NULL);
	...
	| value {
			...
			$$ = expression_init(NULL, NULL, NULL, value);
	}
	;
